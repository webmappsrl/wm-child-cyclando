...
add_filter( 'facetwp_facet_filter_posts', function( $return, $params ) {
    ...
    if ( 'dove_vuoi_andare2' == $params['facet']['name'] ) 
    {
        $sanitized = $selected_values;
        global $wpdb;
        // get post IDs of available rooms between these dates
        $post_ids =  $wpdb->get_results( "SELECT post_id FROM {$wpdb->prefix}facetwp_index WHERE facet_display_value LIKE '%$sanitized%'" );
        $mapped = array_map(function($i){return intval($i->post_id);},$post_ids);
        add_filter( 'facetwp_filtered_post_ids', function( $post_ids, $class ) use ($mapped){
            return $mapped;
        }, 10, 2 );
        return $mapped;
    }
