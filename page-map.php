	...
	while (have_posts()) {
		...
		if ($content_area_id != '' and get_post_status($content_area_id) != FALSE) {
			...
			if ($site_url == 'http://cyclando.local') {
				$site_url = 'http://localhost/Redrum';
			} else {
				$site_url = 'https://apiredrum.reflexproject.com';
			}
			$domande = file_get_contents($site_url . '/psql/api/post/read_all_domande.php');
	?>
			<div class="redrum" style="width:100%;height:100%;">
				<div class="questions" style="width:30%;float:left;overflow:auto;height:100vh;">
					<?php echo $domande; ?>
				</div>
				<div style="height:100vh;width:70%;float:left;">
					<?php
			...
			<script type="text/javascript" language="javascript">
				jQuery(document).ready(function($) {
					...
					if (siteurl == 'http://cyclando.local') {
						siteurl = 'http://localhost/Redrum';
					} else {
						siteurl = 'https://apiredrum.reflexproject.com';
					}
					...
					function onDomandaClick(e) {
						...
						$.ajax({
							type: "GET",
							url: siteurl + "/psql/api/post/read_domanda.php",
							data: "id=" + domandaid,
					...
					function onVarianteClick(e) {
						...
						$.ajax({
							type: "GET",
							url: siteurl + "/psql/api/post/read_variante.php",
							data: "id=" + varianteid,
							success: function(data) {
								try {
									console.log(data);
